...
def services(call, service_id):
    conn = sqlite3.connect('C:\\sqlite\\test.db')
    c = conn.cursor()

    # Fetch all services from the database
    c.execute("SELECT * FROM services WHERE category_id = ?", (service_id,))
    services = c.fetchall()
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Select a service:", reply_markup=markup)
...
@bot.callback_query_handler(func=lambda call: call.data in ["shop", "services", "details"])
def main_menu_callback(call):
    if call.data == "details":
        ...
        categories(call.message.chat.id)
    elif call.data == "help":
        bot.send_message(call.message.chat.id, "You selected option 3.")

@bot.callback_query_handler(func=lambda call: call.data == "main_menu")
def return_main_menu(call):
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Main menu:", reply_markup=main_menu_markup)
